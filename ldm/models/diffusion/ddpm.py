    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        if exists(given_betas):
            ...
            betas = make_beta_schedule(beta_schedule, timesteps, linear_start=linear_start, linear_end=linear_end,
                                       cosine_s=cosine_s)
        alphas = 1. - betas
        alphas_cumprod = np.cumprod(alphas, axis=0) # cumprod:累乘序列
...
class LatentDiffusion(DDPM):
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    # log_images
                    c = self.get_learned_conditioning(xc,x)
                else:
                    c = self.get_learned_conditioning(xc.to(self.device),x)
            else:
                # training
                c = xc
            if bs is not None:
    ...
    def forward(self, x, c, img, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key is not None:
            assert c is not None
            # c = text
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c, img)
            # c = tensor embed
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation",
                ...
                print('self.cond_stage_key in [image]')
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.parameterization == "x0":
            ...
            raise NotImplementedError()
        # target = x_start
        # t_next = torch.tensor([int(t)+1]).to(self.device)
        # target = self.q_sample(x_start=x_start, t=t_next, noise=noise)
        ...
        if self.embedding_reg_weight > 0:
            ...
            print('loss_embedding_reg',self.embedding_reg_weight * loss_embedding_reg)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if sample:
            ...
            uc = self.get_learned_conditioning(len(c) * [""], x)
        ...
        if plot_progressive_rows:
            with self.ema_scope("Plotting Progressives"):
                img, progressives = self.progressive_denoising(c,
                                                               shape=(self.channels, self.image_size, self.image_size),
                                                               batch_size=N)
            prog_row = self._get_denoiembedding_parametersse_row_from_list(progressives, desc="Progressive Generation")
